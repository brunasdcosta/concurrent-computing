...
import jcstress.Test.StressTest;
import org.openjdk.jcstress.infra.results.I_Result;
import jcstress.Test.MyState;

public final class Test_StressTest_jcstress extends Runner<I_Result> {
    ...
    public Test_StressTest_jcstress(ForkedTestConfig config) {
        super(config);
    }
    ...
    private Counter<I_Result> task_actor() {
        int len = config.strideSize * config.strideCount;
        ...
        while (true) {
            ...
            for (int start = 0; start < len; start += stride) {
                run_actor(gs, gr, start, start + stride);
                check += 1;
    ...
    private void run_actor(MyState[] gs, I_Result[] gr, int start, int end) {
        StressTest lt = test;
        MyState[] ls = gs;
        I_Result[] lr = gr;
